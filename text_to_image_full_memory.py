# CUDA_VISIBLE_DEVICES=2,3 accelerate launch --config_file accelerator_gpu.json  --mixed_precision="fp16"  text_to_image_full_memory.py --snr_gamma=5.0  --pretrained_model_name_or_path="CompVis/stable-diffusion-v1-4" --dataset_name="mimic-cxr" --train_batch_size=20  --gradient_accumulation_steps=1 --gradient_checkpointing --max_train_steps=10000 --learning_rate=0.0001  --lr_scheduler="constant" --lr_warmup_steps=0 --output_dir="./checkpoints/mimic-mlp-0001"
#!/usr/bin/env ytphon
# find pid port by
# lsof -i:29500
# coding=utf-8
...
def TextEncoderMemory(paths , base, appendix):
    """
...
def VAEMemory(paths , base, appendix):
    """
    Load and convert a batch of latent variables stored in .npy files to a PyTorch tensor.
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.dataset_name  in ["mimic-cxr"]:
            # read sentece.txt and img_path.txt and save each line in a list
            all_images = []
            all_text = []
            all_labels = []
            ...
            dataset = {}
            dataset["train"] = Dataset.from_dict({"path": all_path,"text":all_text,"labels":all_labels})
            print("+"*100)
            print(f"Custome dataset {args.dataset_name} loaded")
            print("+"*100)
    ...
    def collate_fn(examples):
        ...
        vae_latents = VAEMemory(path, "/local/data/vae_latent/files/mimic-cxr-jpg/2.0.0/", ".jpg_latent.npy")
        text_encoder_latents = TextEncoderMemory(path, "/local/home/hhamidi/t_dif/diffusers/examples/text_to_image/dataset/", ".npy")

        return {

            "vae_latents": vae_latents,
            "text_encoder_latents": text_encoder_latents,
            "path": path
        }
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                latents = batch["vae_latents"].to(weight_dtype)
                latents = latents * vae.config.scaling_factor


                # see this part:
                # encoded_vae = vae.encode(img_3channel).latent_dist.sample()
                # # latents = latents * vae.config.scaling_factor
                # encoded_vae = encoded_vae * vae.config.scaling_factor
                # # z = 1 / scaling_factor * z
                # encoded_vae = 1 / vae.config.scaling_factor * encoded_vae
                ...
                encoder_hidden_states = batch["text_encoder_latents"].to(weight_dtype)
